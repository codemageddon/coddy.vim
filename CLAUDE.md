...
- **`plugin/`** — one file per plugin, each self-contained: `vim.pack.add()` + `setup()` + keymaps together.
  ...
  - `lsp-file-operations.lua` — `antosha417/nvim-lsp-file-operations`, hooks neo-tree rename/move/delete events into LSP `workspace/willRenameFiles` etc. so imports update automatically.
  - `window-picker.lua` — `s1n7ax/nvim-window-picker`, `<leader>wp` to jump to a picked window.
